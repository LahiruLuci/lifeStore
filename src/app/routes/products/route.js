...
export async function GET(){
    try {
        ...
        const query = "SELECT p.PRODUCTID, p.PRODUCTTITLE, p.PRODUCTNAME, p.PRODUCTCODE, p.PREDESCRIPTION, p.DESCRIPTIONTITLE, p.DESCRIPTION, p.IMAGELOCATION, p.STATUS, p.SORTID, p.CREATEDDATETIME, p.LASTUPDATEDDATETIME, p.CREATEDUSER, p.LASTUPDATEDUSER, GROUP_CONCAT(CASE WHEN pf.PRODUCTFEATURECATEGORY = '5' THEN pf.DESCRIPTION END SEPARATOR ' | ') AS MAINPRODUCTFEATURES, GROUP_CONCAT(CASE WHEN pf.PRODUCTFEATURECATEGORY = '4' THEN pf.DESCRIPTION END SEPARATOR ' | ') AS PRODUCTFEATURES, ppm.AMOUNT FROM product p LEFT JOIN productfeature pf ON p.PRODUCTID = pf.PRODUCT LEFT JOIN productpaymentmethod ppm ON p.PRODUCTID = ppm.PRODUCT GROUP BY p.PRODUCTID, p.PRODUCTTITLE, p.PRODUCTNAME, p.PRODUCTCODE, p.PREDESCRIPTION, p.DESCRIPTIONTITLE, p.DESCRIPTION, p.IMAGELOCATION, p.STATUS, p.CREATEDDATETIME, p.LASTUPDATEDDATETIME, p.CREATEDUSER, p.LASTUPDATEDUSER, ppm.AMOUNT ORDER BY p.SORTID ASC;";
